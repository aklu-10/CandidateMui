...
import { handleRadioChange } from "../handlers/RadioHandlers";
import { handleCreatableSelectChange } from "../handlers/SelectHandlers";
import { handleSelectChange } from "../handlers/SelectHandlers";
...
const CandidateTestForm = ({ formUpdationKey }) => 
{
    ...
    const [validation, setValidation] = useState({
        ...
        regex: "[^0-9][^!@#$%]+",
        errMsg:
            "Test name must not contain special character and not start with number",
        isValid: true,
        },
        total_no_question: {
        regex: "^[1-9]d*",
        ...
        regex: "^[0-9]d*",
        errMsg:
            "Total random questions must be greater than zero and less than total",
        isValid: true,
        },
        no_of_predefined_questions: {
        regex: "^[0-9]d*",
        errMsg:
            "Total predefined questions must be greater than zero and less than total",
        isValid: true,
        },
        technologies: {
        regex: "^[0-9]d*",
        errMsg:
            "Please provide a value which is less than total random questions (zero excluded) ",
        isValid: true,
        },
    });
...
return (
    ...
    value={{
        ...
        setShowAddNewQuestionForm,
        fetchRows,
        showAddTechnologyButton,
        setShowAddTechnologyButton,
        setSelectedRows,
        selectedRows
    }}
    >
    ...
    <div className="mb-[15px] p-2 flex flex-row items-center relative">
        ...
        <TextField
            id="outlined-basic"
            value={masterData.test_types[formUpdationKey].test_name}
            variant="outlined"
            size="small"
            name="test_name"
            onChange={(e) =>
        ...
        {formUpdationKey === "form1" ? (
            ...
            <Button
            variant="contained"
            onClick={() => handleDeleteTestForm(formUpdationKey)}
            >
    ...
    <div className="mb-[15px] p-2 w-[80%]">
        ...
        defaultValue={masterData.test_types[formUpdationKey].test_type_key}
        onChange={(value) =>
            handleCreatableSelectChange("test_type_key", value, {masterData, setMasterData, formUpdationKey})
        }
        />
    </div>
    ...
    <div className="mb-[15px] p-2 flex flex-row">
        ...
        <Select
            ...
            value: masterData.test_types[formUpdationKey].is_for_agent_panel,
            }}
            options={[
        ...
        <RadioGroup
            ...
            onChange={(e) => handleRadioChange("is_mcq", e,{masterData, setMasterData, formUpdationKey})}
        >
            {masterData.test_types[formUpdationKey].is_for_agent_panel ? (
            <>
                <FormControlLabel
    ...
    <div className="mb-[15px] p-2 w-[80%]">
        ...
        defaultValue={{
            label: `${
            masterData.test_types[formUpdationKey].is_screening_test === 0
                ? "Pre Interview"
                : "Post Interview"
            }`,
            value: masterData.test_types[formUpdationKey].is_screening_test,
        }}
        options={[
            { label: "Pre Interview", value: 0 },
            { label: "Post Interview", value: 1 },
        ]}
        onChange={(value) => handleSelectChange("is_screening_test", value, {masterData, setMasterData, formUpdationKey})}
        />
    </div>
    ...
    <div className="mb-[15px] p-2 flex flex-col w-[80%]">
        ...
        size="small"
        variant="outlined"
        name="total_no_question"
        value={masterData.test_types[formUpdationKey].total_no_question}
        onChange={(e) =>
            handleInputChange(e, {
            validation,
            setValidation,
        ...
        <div className="flex flex-row items-center gap-[2px] my-1">
            <InfoIcon color="error" style={{ fontSize: "1.4rem" }} />
            <span className="text-red-500">
            {validation.total_no_question.errMsg}
            </span>
        </div>
        )}
    ...
    {masterData.test_types[formUpdationKey].test_name &&
        ...
        <TabSwitch
            tabs={[
            { label: "Random Questions", value: <RandomQuestion /> },
            { label: "Predefined Questions", value: <PredefinedQuestion /> },
            ]}
        />
    )}
    </ValidationContext.Provider>
